...
def merge_files(data_dir, shuffle=False, max_len=1022, save_path=None):
...
def split_data(protein, train_size=0.8, shuffle=False, n_sites=None, neg_train=False,
    ...
    if train_ids is not None:
        ...
        if train_size < 1:
            ...
            # 当train_size是绝对数量时（例如20）
            train_size = int(train_size)  # 确保是整数
            if shuffle:
                df = df.sample(frac=1)  # 如果需要打乱数据

            if n_sites is not None:
                n_sites = set(n_sites)

            # 创建布尔筛选条件（与原逻辑一致）
            df_bool = df.apply(lambda row:
                               (not n_sites or len(row['positions']) in n_sites) and
                               (not neg_train or row['DMS_score_bin'] == 0),
                               axis=1)

            # 筛选后取前N条作为训练集
            filtered_df = df.loc[df_bool]
            train['df'] = filtered_df.iloc[:train_size]

            # 剩余数据作为测试集（包括不符合筛选条件的数据）
            test['df'] = df.loc[df.index.difference(train['df'].index, sort=False)]
