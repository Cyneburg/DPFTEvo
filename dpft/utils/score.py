...
metrics = ['max_activity', 'ndcg', 'topk_pr']
...
def compute_scores(predicts, targets, labels, k=20):
    ...
    report['max_activity'] = targets[indices].max().item()
    report['topk_pr'] = torch.count_nonzero(labels[indices]).item() / k
    return report

def group_scores(train_df, pred_df, test_df, k=20):
